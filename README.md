...
2. Start the Gateway and keep its random token private:
...
4. Install the APK from the latest release, open **Settings**, add a profile using your `wss://` endpoint, paste the token, and tap **Apply & Save**.

The app should show the Workspaces and Tabs currently available in cmux. See the [networking and setup guide](docs/networking.md) for LAN, reverse-proxy, tunnel, VPN, token, and troubleshooting details.
...
Requirements: JDK 17, Android SDK 35, and `uv` for the Gateway's isolated Python dependencies.
